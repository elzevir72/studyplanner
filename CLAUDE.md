...
- 대시보드의 "오프라인 모임" 뷰는 고정 간격(격주 등)을 가정하지 않는다. 관리자가 실제 모임이 열린 날짜를 등록(`Meetings`)하면 그 날짜들을 기준으로 회차가 자동으로 매겨진다 — 실제 모임 주기가 불규칙할 수 있어서(밀리거나 당겨짐), 날짜 계산이 아니라 관리자가 등록한 실제 날짜를 그대로 anchor로 쓴다.
